...
import { IVehicle } from "src/interface/IVehicle";
import { IStarship } from "src/interface/IStarship";
import { IPlanet } from "src/interface/IPlanet";
import { ISpecies } from "src/interface/ISpecies";
...
export class FrontendView {
        ...
        if (this.HTMLTable) {
            ...
            if (Array.isArray(value)) {
                ...
                if (key != 'Homeworld' && key != 'Films' && key != 'Vehicles' && key != 'Starships'){
                    ...
                    let ul = this.createElement('ul', ['list-group', 'm-1']);
                        let li : HTMLElement = this.createElement('li', ['list-group-item', key]);
                        ul.appendChild(li);
                        td.appendChild(ul);
                }
            }
    ...
    public fillPeopleTable = (person: IPerson) => {

        let field : HTMLCollectionOf<HTMLElement>;
        ...
        field = document.getElementsByClassName('Homeworld') as HTMLCollectionOf<HTMLElement>;
        ...
        if (Array.isArray(person.films)){
            field = document.getElementsByClassName('Films') as HTMLCollectionOf<HTMLElement>;
        ...
        if (Array.isArray(person.species)){
            field = document.getElementsByClassName('Species') as HTMLCollectionOf<HTMLElement>;
        ...
        if (Array.isArray(person.vehicles)){
            field = document.getElementsByClassName('Vehicles') as HTMLCollectionOf<HTMLElement>;
        ...
        if (Array.isArray(person.starships)){
            field = document.getElementsByClassName('Starships') as HTMLCollectionOf<HTMLElement>;
    ...
    public fillHomeworldTable = (homeworld: IPlanet) => {
    ...
    public fillFilmTable = (film: IFilm) => {
    ...
    public fillSpeciesTable = (species: ISpecies) => {
    ...
    public fillVehicleTable = (vehicle: IVehicle) => {
    ...
    public fillStartshipTable = (starship: IStarship) => {
