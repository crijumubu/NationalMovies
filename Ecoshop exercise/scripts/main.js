...
function displayPage(pageNumber, gridContainer, displayArrData){
    ...
    for (let i=loopStart; i < loopFinish; i++){
        displayProduct(displayArrData[i], gridContainer);
    }

}

function removeCardsGrid(gridContainer){
...
function addCart(){
    ...
    for (const button of cartButton){
        ...
        button.addEventListener('click', (event) => {
            ...
            let productIdentificator = event.target.parentElement.parentElement.previousElementSibling.previousElementSibling.innerText;
...
function pagination(pageNumber, displayArrData, gridContainer){

    displayPage(pageNumber, gridContainer, displayArrData);
    ...
    do{
        ...
        buttonPagination.addEventListener('click', function () {
            ...
            if (buttonPaginationContent == "&gt;"){
                ...
                pagination(initPageNumber, displayArrData, gridContainer);
            }
                ...
                pagination(limitPageNumber, displayArrData, gridContainer);

            }else{

                pageNumber =  parseInt(buttonPagination.innerHTML);
                pagination(pageNumber, displayArrData, gridContainer);
            }
        });
...
function btnsAction(gridContainer){

    const btnInit = document.getElementsByClassName('btnInit')[0];
    
    btnInit.addEventListener("click", () => {
        removeCardsGrid(gridContainer);
        pagination(1, data, gridContainer);
    });
    ...
    btnShowFavorites.addEventListener("click", () => {
        ...
        if (favorites.length != 0){
            pagination(1, favorites, gridContainer);
        }else{
...
function main(){
    ...
    pagination(startPage, data, gridContainer);
    btnsAction(gridContainer);
    filter(gridContainer);
}
